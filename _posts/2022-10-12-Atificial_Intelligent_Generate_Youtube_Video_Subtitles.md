...
python依赖过多，需要安装的各种pip插件依赖项太多，因为这个功能大部分是基于Python的，后期我会把这个东西转移到c++为底层的，应用层以C#为主要技术栈进行重构，那样子代码质量和性能更加可靠。安装的时候也不需要去配置那么多的python生态的依赖项目。这套工具之所以没有普及或者说对于初学者非常不友好的地方就是需要命令行配置太多太多东西。
