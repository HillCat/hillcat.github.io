...
这个插件的作者是国内的某个计算机大学毕业的算法学生搞出来的一个基于人工智能机器学习的插件，目的是为了优化我们的复习参数。因为anki的复习参数太多了，每个人的学习数据都不一样，不可能一套参数搞定所有人的复习参数。所以这个插件使用到的参数，需要通过机器学习，python在线训练，得到一个比较优化的参数，然后设置到我们的anki中。目前这个参数支持QT6版本的电脑端anki和苹果手机端Anki。可以优化我们的复习参数实现最优解。
...
等待脚本慢慢执行，它正在根据我们自己导出来的anki所有卡片的数据，来进行AI机器学习运算，最终会计算出一个最优化的参数。我们之后要拿到这个结果参数，去替换掉js脚本中的参数，粘贴到我们自己的anki中去。
