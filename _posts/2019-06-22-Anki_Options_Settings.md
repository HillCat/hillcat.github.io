...
比如Flag 1代表的是红色旗子，筛选的方法就是直接在过滤器种输入:`Flag:1`，然后回车，所有被标记过红色旗子的单词都会被筛选出来，你输入的参数它会自动给你加上双引号：
...
一般我常用的就是前面这几种旗子，用来做标记的。难的单词一般都是标记为红色旗子。或者有些单词，你要对它增加笔记内容，复习的时候你就可以标记颜色，等到有空的时候抽空对那些单词进行整理。手机端和PC端，安卓苹果端都是有这些旗子标记功能的。
