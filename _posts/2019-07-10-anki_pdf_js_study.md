...
我曾经看专业材料的时候，遇到很多的专业词汇，在牛津9和韦氏里面都查不到，包括去它们的官网都查不到，但是去到LEXICO就搞定了。收词量，LEXICO真的超级巨无霸。一般不是遇到特别特别超纲词是不会祭出这个大杀器的，因为它的体积太大了。
...
为什么要用图片，而不是文字插入呢？因为anki默认的编辑器会去掉冗余的css样式，导致你贴进去的辞典样式丢失错位，要保留样式可以使用`shift + ctrl +V `快捷键,但是也会有文字样式的丢失，无法做到100%跟辞典里面的排版一样，总有一些错位，各种问题，曾经我尝试自己编写一套程序，让它完全100%保留排版样式，但几乎不可能做到，因为辞典中内容庞大，各种样式的css和html标签都有，情况复杂，特别是收词量越大的词典，这么做很难，所以干脆直接粘贴图片。牺牲掉一些手机端的阅读体验，因为手机端阅读图片会有失真。只要你把欧陆弹出框弄窄一些(截出来图片也是很窄的)，手机上阅读是没有任何问题的，完美!

anki为啥会有这个清除冗余css的动作，我估计还是因为服务器硬盘资源有限，anki毕竟是免费为大家保存数据，云端硬盘都是要付费的，所以它会对用户的数据进行“冗余清除”，去掉部分css，减轻服务器硬盘开销。从使用2年的anki来看，我认为这是目前我能想到的最好的办法。
