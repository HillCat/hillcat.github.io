...
注意，如上图：~~Video(mp4) 这个字段可以不做映射，使用`<ignored>`即可~~，去掉这个字段，手机上的卡片无法显示视频。貌似webm给电脑显示，mp4给手机显示的。这个模板其实是直接Copy的老外的模板。

说明：等所有设置完成了之后，模板自带的Deck可以删掉，删的时候，apkg包导入进来的1000多张卡会被清空(删Deck，不会影响Note Type，因为Note Type只能通过进入Anki的Browse里面去删除)。
