...
namespace FakeDataGenerator
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var detectionLoadTestList = new Faker<DetectionLoadTestEntity>()
                .RuleFor(d=>d.ThirdUnitDto,f=>f.PickRandom(thirdUnitIdList))
                .RuleFor(d => d.BgeId,f=> f.Random.Int(1, 27))
            ...
            var detectiontestList = new Faker<DetectionStaticTest>()
                .RuleFor(d => d.BgeId, f=>f.Random.Int(1,27))
            ...
            //for (int i = 0; i < 4; i++)
            //{
                var detectionLoadTest = detectionLoadTestList.Ignore(p => p.Id).Generate(100);
            ...
            //for (int i = 0; i < 4; i++)
            //{
            //    var detectionDtoList = detectiontestList.Ignore(p => p.Id).Generate(50000);

            //    var text = JsonSerializer.Serialize(detectionDtoList, options: new JsonSerializerOptions()
            //    {
            //        Encoder = JavaScriptEncoder.Create(UnicodeRanges.All)
            //    });
            //    var path = "c:/temp/statictest" + i + ".json";
            //    File.WriteAllText(path, text);
            //}
